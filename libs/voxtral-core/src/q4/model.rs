...
//!
//! Mirrors the f32 model in `inference/bf16/` but uses [`Q4Linear`] for all
...
use crate::layers::kv_cache::{KVCache, LayerCaches};
use crate::layers::masking::{
...
// ---------------------------------------------------------------------------
// reshape_encoder_output
// ---------------------------------------------------------------------------

/// Reshape encoder output for the adapter.
///
/// Concatenates `reshape_factor` consecutive encoder frames into one vector,
/// reducing the sequence length by that factor while increasing the hidden dim.
///
...
/// uses regular Burn matmuls (activation x activation).
...
impl Q4Attention {
    ...
    fn expand_kv(
        &self,
        k: Tensor<WgpuBackend, 4>,
        v: Tensor<WgpuBackend, 4>,
    ) -> (Tensor<WgpuBackend, 4>, Tensor<WgpuBackend, 4>) {
...
impl Q4FeedForward {
    ...
    /// Forward pass.
    pub fn forward(&self, x: Tensor<WgpuBackend, 3>) -> Tensor<WgpuBackend, 3> {
...
impl Q4AdaRmsNorm {
    ...
    pub fn forward(&self, x: Tensor<WgpuBackend, 3>, t_embed: Tensor<WgpuBackend, 3>) -> Tensor<WgpuBackend, 3> {
...
pub struct Q4EncoderLayer {
    attention_norm: RmsNorm<WgpuBackend>,
    attention: Q4Attention,
    ffn_norm: RmsNorm<WgpuBackend>,
    ffn: Q4FeedForward,
}
...
pub struct Q4AudioEncoder {
    conv: ConvDownsampler<WgpuBackend>,
    rope: RoPE<WgpuBackend>,
...
impl Q4AudioEncoder {
    ...
    /// * `offset` - Position offset for KV cache
    pub fn forward(&self, mel: Tensor<WgpuBackend, 3>, offset: usize) -> Tensor<WgpuBackend, 3> {
    ...
    pub fn create_cache(&self) -> LayerCaches<WgpuBackend> {
...
impl Q4LanguageModel {
    ...
    pub fn new(
        tok_embeddings: Tensor<WgpuBackend, 2>,
        rope: RoPE<WgpuBackend>,
        layers: Vec<Q4DecoderLayer>,
        norm: RmsNorm<WgpuBackend>,
    ) -> Self {
    ...
    pub fn embed_tokens(&self, token_ids: Tensor<WgpuBackend, 2, Int>) -> Tensor<WgpuBackend, 3> {
    ...
    pub fn embed_tokens_from_ids(
        ...
        match &self.tok_embeddings {
            TokEmbedStore::F32(embed) => {
                let id_tensor = Tensor::<WgpuBackend, 2, Int>::from_data(
    ...
    pub fn forward(
        &self,
        token_ids: Tensor<WgpuBackend, 2, Int>,
        t_embed: Tensor<WgpuBackend, 3>,
        offset: usize,
    ) -> Tensor<WgpuBackend, 3> {
    ...
    pub fn forward_hidden(
        &self,
        hidden_states: Tensor<WgpuBackend, 3>,
        t_embed: Tensor<WgpuBackend, 3>,
        offset: usize,
    ) -> Tensor<WgpuBackend, 3> {
    ...
    pub fn lm_head(&self, hidden_states: Tensor<WgpuBackend, 3>) -> Tensor<WgpuBackend, 3> {
        match &self.tok_embeddings {
            TokEmbedStore::F32(embed) => {
    ...
    pub fn create_cache_preallocated(&self, max_seq: usize) -> LayerCaches<WgpuBackend> {
...
impl Q4VoxtralModel {
    ...
    pub fn encode_audio(&self, mel: Tensor<WgpuBackend, 3>) -> Tensor<WgpuBackend, 3> {
    ...
    pub fn forward_with_cache(
        ...
        t_embed_decoder: Tensor<WgpuBackend, 3>,
        encoder_cache: &mut LayerCaches<WgpuBackend>,
        decoder_cache: &mut LayerCaches<WgpuBackend>,
    ) -> Tensor<WgpuBackend, 3> {
    ...
    ///
    /// Runs a full prefill+decode loop: encodes audio, creates a fixed prefix
    /// of BOS + padding tokens, prefills the decoder, then autoregressively
    /// decodes one token per audio position.
    pub fn transcribe_streaming(
        ...
        // Note: DIAG_LOGITS env var not available in WASM, diagnostics only in native build
        ...
        // noise in Q/K projections creates correlated errors across positions
        // that amplify through softmax, destroying hidden states by layer ~6.
        // Single-token (M=1) decode is numerically stable.
        let hidden = {
            let _prefill = tracing::info_span!("prefill").entered();
            let mut last_hidden = prefix_inputs.clone().slice([0..1, 0..1, 0..d_model]);
            for pos in 0..PREFIX_LEN {
                let input = prefix_inputs.clone().slice([0..1, pos..pos + 1, 0..d_model]);
                last_hidden = self.decoder.forward_hidden_with_cache(
                    input,
        ...
        // hidden is already [1, 1, d_model] from single-token decode
        let first_pred = logits.clone().argmax(2);
        let first_token: i32 = first_pred.into_scalar().elem();

        // Diagnostic logits removed (batch prefill replaced with sequential)
        ...
        let audio_slices: Vec<Tensor<WgpuBackend, 3>> = (PREFIX_LEN..seq_len)
    ...
    /// Async streaming transcription with KV cache (WASM-safe).
    ///
    /// Same logic as [`transcribe_streaming`](Self::transcribe_streaming) but
    /// replaces all synchronous GPU readback (`into_scalar()`, `into_data()`)
    /// with `into_data_async().await`, which is required on WASM because
    /// WebGPU cannot do synchronous GPU→CPU readback.
    ///
    /// Diagnostic readbacks are skipped on WASM to avoid massive GPU->CPU
    /// transfers that stall the pipeline.
    pub async fn transcribe_streaming_async(
        &self,
        mel: Tensor<WgpuBackend, 3>,
        t_embed_decoder: Tensor<WgpuBackend, 3>,
    ) -> Result<Vec<i32>, String> {
        let _span = tracing::info_span!("transcribe_streaming_async").entered();
        ...
        if seq_len < PREFIX_LEN {
            eprintln!(
                "[Q4Model] seq_len {} < PREFIX_LEN {}, returning empty",
                seq_len, PREFIX_LEN
            );
            return Ok(Vec::new());
        }
        ...
        eprintln!(
            "[Q4Model] transcribe_streaming_async: seq_len={} d_model={} decode_steps={}",
            seq_len, d_model, seq_len - PREFIX_LEN - 1
        );
        ...
        // Diagnostic readbacks only on native — skip on WASM to avoid GPU stalls
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let data = test_embeds
                .into_data_async()
                .await
                .map_err(|e| format!("GPU readback failed (embed diag): {:?}", e))?;
            let vals = data.as_slice::<f32>().unwrap_or(&[]);
            for (i, &id) in test_ids.iter().enumerate() {
                ...
                eprintln!(
                    "[Q4Model] embed_norm: token {} → L2={:.4} nonzero={}/{}",
                    id, norm, nonzero, d_model
                );
            }
        }
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let data = flat
                .into_data_async()
                .await
                .map_err(|e| format!("GPU readback failed (prefix diag): {:?}", e))?;
            let vals = data.as_slice::<f32>().unwrap_or(&[]);
            ...
            let first10: Vec<String> = vals[d_model..d_model + 10.min(vals.len())]
                .iter()
                .map(|v| format!("{:.4}", v))
                .collect();
            eprintln!(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let data = flat
                .into_data_async()
                .await
                .map_err(|e| format!("GPU readback failed (prefix_inputs diag): {:?}", e))?;
            let vals = data.as_slice::<f32>().unwrap_or(&[]);
            let nonzero = vals.iter().filter(|&&v| v.abs() > 1e-10).count();
            let min_v = vals.iter().cloned().fold(f32::INFINITY, f32::min);
            let max_v = vals.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
            eprintln!(
                "[Q4Model] prefix_inputs (audio+text): nonzero={}/{} min={:.4} max={:.4}",
                nonzero,
                vals.len(),
                min_v,
                max_v
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let last_hidden = hidden
                .clone()
                .slice([0..1, (hseq - 1)..hseq, 0..hdim])
                .reshape([hdim]);
            let data = last_hidden
                .into_data_async()
                .await
                .map_err(|e| format!("GPU readback failed (hidden diag): {:?}", e))?;
            let vals = data.as_slice::<f32>().unwrap_or(&[]);
            let nonzero = vals.iter().filter(|&&v| v.abs() > 1e-10).count();
            let min_v = vals.iter().cloned().fold(f32::INFINITY, f32::min);
            let max_v = vals.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
            let mean = vals.iter().sum::<f32>() / vals.len().max(1) as f32;
            let first10: Vec<String> = vals[..10.min(vals.len())]
                .iter()
                .map(|v| format!("{:.4}", v))
                .collect();
            eprintln!(
        ...
        // hidden is [1, 1, d_model] from single-token prefill — logits are [1, 1, vocab]
        let first_pred = logits.clone().argmax(2);
        let first_token_data = first_pred
            .into_data_async()
            .await
            .map_err(|e| format!("GPU readback failed (first token): {:?}", e))?;
        let first_token: i32 = first_token_data
            .to_vec::<i32>()
            .map_err(|e| format!("first token extraction failed: {:?}", e))?[0];

        // Diagnostic logits removed (used stale variable names from batch prefill)

        eprintln!(
            "[Q4Model] prefill done, first_token={}, starting decode ({} steps)",
            first_token,
        ...
        for pos in (PREFIX_LEN + 1)..seq_len {
            ...
            let data = pred
                .into_data_async()
                .await
                .map_err(|e| format!("GPU readback failed (decode step {}): {:?}", pos, e))?;
            let next_token: i32 = data
                .to_vec::<i32>()
                .map_err(|e| format!("token extraction failed (step {}): {:?}", pos, e))?[0];
            generated.push(next_token);
        }
    ...
    pub fn create_encoder_cache(&self) -> LayerCaches<WgpuBackend> {
        self.encoder.create_cache()
    }

    /// Create KV caches for the decoder.
    pub fn create_decoder_cache(&self) -> LayerCaches<WgpuBackend> {
        self.decoder.create_cache()
    }

    /// Create pre-allocated KV caches for the decoder.
    pub fn create_decoder_cache_preallocated(&self, max_seq: usize) -> LayerCaches<WgpuBackend> {
