...
mod tests {
    ...
    #[test]
    fn test_max_source_positions_null() {
        // When max_source_positions is null in JSON, it should default to Some(1500)
        let json = r#"{"max_source_positions": null}"#;
        let v: serde_json::Value = serde_json::from_str(json).unwrap();
        let config = AudioEncoderConfig::from_json_value(&v);
        assert_eq!(config.max_source_positions, Some(1500));
    }

    #[test]
    fn test_max_source_positions_set() {
        let json = r#"{"max_source_positions": 3000}"#;
        let v: serde_json::Value = serde_json::from_str(json).unwrap();
        let config = AudioEncoderConfig::from_json_value(&v);
        assert_eq!(config.max_source_positions, Some(3000));
    }

    #[test]
    fn test_effective_max_positions() {
        let config = AudioEncoderConfig {
            max_source_positions: Some(1500),
            sliding_window: 750,
            ..Default::default()
        };
        // 1500/4 = 375, min(375, 750) = 375
        assert_eq!(config.effective_max_positions(), 375);

        let config_large_window = AudioEncoderConfig {
            max_source_positions: Some(6000),
            sliding_window: 750,
            ..Default::default()
        };
        // 6000/4 = 1500, min(1500, 750) = 750
        assert_eq!(config_large_window.effective_max_positions(), 750);

        let config_no_max = AudioEncoderConfig {
            max_source_positions: None,
            sliding_window: 750,
            ..Default::default()
        };
        assert_eq!(config_no_max.effective_max_positions(), 750);
    }

    #[test]
    fn test_audio_timing_calculations() {
        let config = AudioInputConfig::default();
        // 15 seconds of audio at 16kHz = 240000 samples = 1500 mel frames
        assert_eq!(config.max_samples(1500), 240000);
        assert!((config.max_duration_secs(1500) - 15.0).abs() < 1e-6);
        assert_eq!(config.mel_frames_for_samples(240000), 1500);
    }
}
