...
async fn request_adapter(
    ...
    let mut instance_flags = InstanceFlags::advanced_debugging();
    // PATCH: allow non-conformant Vulkan adapters (e.g. Mesa DZN on WSL2) when
    // WGPU_ALLOW_UNDERLYING_NONCOMPLIANT_ADAPTER=1. Without this flag, dzn is
    // invisible to enumerate_adapters / request_adapter on wgpu 26.
    if std::env::var("WGPU_ALLOW_UNDERLYING_NONCOMPLIANT_ADAPTER")
