...
impl DecoderAttention {
    fn load(prefix: &str, vb: &VarBuilder) -> Result<Self> {
        ...
        let wq = linear_no_bias(d_model, q_dim, vb.pp(&format!("{prefix}.attention.wq")))?;
        let wk = linear_no_bias(d_model, kv_dim, vb.pp(&format!("{prefix}.attention.wk")))?;
        let wv = linear_no_bias(d_model, kv_dim, vb.pp(&format!("{prefix}.attention.wv")))?;
    ...
    fn forward_with_cache(&self, x: &Tensor, rope: &RoPE, cache: &mut KVCache) -> Result<Tensor> {
        ...
        let v = self.wv.forward(x)?.reshape((b, seq, self.n_kv_heads, self.head_dim))?;
        ...
        // Manual attention with GQA expand + causal mask
        let q = q.transpose(1, 2)?.contiguous()?;
        ...
        let scores = (q.matmul(&k.transpose(2, 3)?.contiguous()?)? * (self.scale as f64))?;
...
impl VoxtralModel {
    pub fn load(safetensors_path: &std::path::Path, device: &Device) -> Result<Self> {
        ...
        eprintln!("[CandleNative] Token embedding on GPU ({:.0} MB, bf16)",
            131072.0 * 3072.0 * 2.0 / 1e6);

        eprintln!("[CandleNative] Full model loaded in {:.1}s ({:?})", t0.elapsed().as_secs_f32(), dtype);
        Ok(Self { encoder, adapter, decoder_layers, decoder_norm, decoder_rope, tok_embed, device: device.clone() })
    }
    ...
    /// lm_head: hidden @ tok_embed^T → logits
    pub fn lm_head(&self, hidden: &Tensor) -> Result<Tensor> {
        // hidden [B, seq, D] @ embed_T [D, V] → need [1, D, V] for 3D matmul
        let embed_t = self.tok_embed.t()?.contiguous()?.unsqueeze(0)?;
...
pub fn transcribe(model: &VoxtralModel, mel: &Tensor, t_embed: &Tensor) -> Result<Vec<u32>> {
    ...
    let mut caches: Vec<KVCache> = (0..26).map(|_| KVCache::new()).collect();
    ...
    for i in prompt_len..audio_seq {
        ...
        let token = logits_f32.iter().enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal)).map(|(i, _)| i as u32).unwrap_or(0);
