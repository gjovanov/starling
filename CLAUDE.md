...
- Frontend playback uses the **Web Audio API**, not MSE — Chrome doesn't support `audio/wav` MSE. `frontend/js/modules/tts-player.js` schedules `AudioBufferSourceNode`s for sample-accurate stitching. State machine: `idle → buffering → playing → paused/ended`. State exposed via `window.__ttsPlayerState` for Playwright assertions. The player has four production-tested audio-quality fixes layered on top of the basic schedule:
  1. **Byte-alignment carry** — `fetch().body.getReader()` gives chunks at network-packet boundaries, not 16-bit-sample boundaries. `_pcmTail` carries the residual odd byte forward; without it every odd-byte split shifts subsequent samples by 1 byte and decodes as crackling noise.
  2. **No-skip scheduling** — `start(when)` with `when < currentTime` skips into the buffer (slicing samples off the head). Always clamp to `max(now, _nextStartAt)`. A brief silence gap is far less perceptible than chopped sample heads.
  3. **Chunk aggregation** — incoming ~400 ms upstream chunks are concatenated into ~500 ms blocks before scheduling, ~5× fewer source boundaries. Reduces resampler-startup transients that otherwise stack into a sub-bass amplitude tremolo.
  4. **Per-block loudness normaliser** — Voxtral itself emits long sentences ~30 dB quieter than short ones (`docs/tts_spike.md` Phase 8 has the measurements). Per-aggregated-block RMS measurement, smoothed gain (45/55 EMA), 8× max boost, soft-clip at ±0.97 with tanh roll-off. Speed control (0.85×–1.5×) via per-source `playbackRate`.
